...
所有可能执行 Git 或写入清单的工作流都先获取工作区隐藏的 `.batchspace.lock`，再以相同固定顺序获取旧版 `.workspace.lock`。兼容锁避免 v1 升级后的客户端与重命名前客户端分裂协调。清单写入先写同目录临时
...
`.batchspace.lock` 以及 v1 兼容 `.workspace.lock`。批量命令不是事务：某些仓库已创建提交后，后续仓库失败或超时不会触发自动
