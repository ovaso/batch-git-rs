...
Every workflow that may run Git or write the manifest uses the hidden workspace `.batchspace.lock`.
...
`.batchspace.lock`. Batch commands are non-transactional: if later repositories fail or time out,
