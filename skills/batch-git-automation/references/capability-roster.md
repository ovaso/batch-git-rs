...
| Missing repository restoration | `restore` | Recreate missing checkouts from the manifest | Local write + network | Does not overwrite existing directories. Failed/timed-out destinations remain for inspection. Invoke only from the workspace root containing `workspace.toml`. |
