...
| 预览与前置条件 | `--plan` → `--apply --expect-workspace-revision <digest>` | 先读取 plan 的范围、风险与 revision；清单改变后重新 plan。apply 不保留远端或工作树状态。 |
...
| 跨仓库合并 | `merge` | 将同一源分支合入各仓库当前分支 | 工作树写入、高风险 | 必须有明确目标、范围和授权；冲突留给人工处理。 |
...
- `sync`、`pull`、`push` 接受精确名称/相对目录、可重复 `--match`，或 `--all`；省略选择器时默认工作区全量。
...
- `--output json` 中，退出码 `1` 仍会提供完整 `data.results[]`；退出码 `2` 读取顶层
  ...
  `branch_ambiguous`、`branch_missing`、`repository_unavailable`、`timeout` 和 `git_exit`。
